...
public class AimBotBallStation {
			...
			try {
				...
				if (hasBall) {
					...
					if (targetContour != null) {
						...
						double r = 0; // Rotation
						double m = 0; // Movement
						double s = 0.8; // Global speed modifier
					...
					// Same as above
					Mat ballStationThreshold = VisionUtils.threshold3Channels(targetHLS, 142, 171, 48, 211, 62, 227);
					...
					if (targetContour != null) {
						...
						if (!fixedControl) {
							...
							if (area < 800) {
								...
								liftingMotor.set(1);
								m = 0;
								// Take away control from the control loop and pickup with a seperate thread
								// This is done because midway through the pickup process we lose the ball station marker so the control loop will be confused
								fixedControl = true;
